...
@Component
public class TvWatcherSilverTrophy implements TrophyInterface {
    private final TrophyRepository trophyRepository;
    private final UserRepository userRepository;
    private final String name = "Tv Show Watcher";
    private final String description = "Watch 10 full tv show";
    private final String image = "";
    private final TrophyType type = TrophyType.SILVER;

    private final KafkaMessageGeneratorService kafkaMessageGeneratorService;

    public TvWatcherSilverTrophy(TrophyRepository trophyRepository,
    ...
    public void checkTrophy(String username, long elementId, TrophyActionName trophyActionName) {
        ...
        Optional<Trophy> trophy = trophyRepository.findByNameAndType(this.name, this.type);
         ...
         if(
                 !trophyActionName.equals(TrophyActionName.ADD_SERIE_IN_WATCHED_LIST)
                 && !trophyActionName.equals(TrophyActionName.REMOVE_SERIE_IN_WATCHED_LIST)
         ){
             return ;
         }
        if (user.getWatchedSeries().size() >= 10 && !user.getTrophy().contains(trophy.get())) {
            user.getTrophy().add(trophy.get());
            userRepository.save(user);
            NotifyUser(username, elementId);
            NotifyFollowers(username, elementId);
        } else if (user.getWatchedSeries().size() < 10 && user.getTrophy().contains(trophy.get())) {
