...
public class MovieWatcherSilverTrophy implements TrophyInterface {
    private final MovieService movieService;
    private final TrophyRepository trophyRepository;
    private final UserRepository userRepository;
    private final String name = "Movie Watcher";
    private final String description = "Watch 10 movies";
    ...
    public MovieWatcherSilverTrophy(MovieService movieService, TrophyRepository trophyRepository, UserRepository userRepository, KafkaMessageGeneratorService kafkaMessageGeneratorService,
    ...
    public void checkTrophy(String username, long elementId) {
        ...
        if (user.getWatchedMovies().size() >= 10 && !user.getTrophy().contains(trophy.get())) {
            user.getTrophy().add(trophy.get());
            userRepository.save(user);
            NotifyUser(username, elementId);
        } else if (user.getWatchedMovies().size() < 10 && user.getTrophy().contains(trophy.get())) {
